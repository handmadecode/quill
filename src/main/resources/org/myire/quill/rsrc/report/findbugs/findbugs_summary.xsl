 ...
 *******************************************************************************
 *
 * XSL style sheet for transforming the summary of a Findbugs XML report into
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="BugCollection">
    ...
    <!-- The FindBugs summary section -->
    <div class="summarysection">

      <!-- Report header -->
      <div class="summaryheader">FindBugs</div>

      <!-- Intro text -->
      <div class="summarysectionitem">
        <span class="summaryintro">
          FindBugs <xsl:value-of select="@version"/> report created
      ...
      <xsl:if test="$num-bugs &gt; 0 and string-length($detailed-report-path) &gt; 0">
        <div class="summarysectionitem">
          <a class="data" href="{$detailed-report-path}" target="_blank">FindBugs details</a>
        </div>
	  </xsl:if>
  ...
  <!-- Output a table with a summary of the Findbugs run -->
